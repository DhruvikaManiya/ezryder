        ...
        @foreach ($order as $order)
            <section class="amount ">
                    ...
                    <div class="amount-left w-53">
                        ...
                        <p class="location loc-pos"><img
                            ...
                            {{ $order->user->area }}, {{ $order->user->city }}, {{ $order->user->pincode }} </p>
                                ...
                                {{-- @dd($order->store) --}}
                                <p class="user"><img
                                    src="{{ asset('asset/images/Vector 3.png') }}"><span>{{ $order->store->name }}</span></p>
                            <p class="location loc-pos"><img
                                    src="{{ asset('asset/images/clarity_map-marker-line.svg') }}">{{   $order->store->address }},{{   $order->Store->area }},<br>
                                 {{   $order->store->city}},<br>
                                 {{   $order->store->state}}, {{ $order->store->country}} </p>
                ...
                <div class="button" id="btns">
                    ...
                    <div class="btn1 fs-20">
                             <a href="{{ route('mobile.delivery.complete_delivery', $order->id) }}">Pickup</div>
