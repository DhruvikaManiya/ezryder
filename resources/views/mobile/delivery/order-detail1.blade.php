            ...
            <div class="dmart_row1">
                <div class="dmart_circle_img">
                    @if($order->store->image != '')
                        <img style="border-radius:50%;width:80%; " src="{{ $order->store->image }}">
                    @else
                        <img style="border-radius:50%;width:80%; " src="{{ $order->store->image }}">
                    @endif
                </div>
                <div class="dmart_address ml-10">
                    <h3>{{ $order->store->area }}</h3>
                    <p class="pt-7">{{ $order->store->address }}</p>
                </div>
                <div>
                ...
                <div class="status status-spn">
                    ...
                    @if ($order->status == 4)
                        <div class="d-flex just-c-e">
                            <button class="theme-bg btn nexBtn fs-25 mb-5 mt-5 btn-w save"><a
                                    href="{{ route('mobile.delivery.d_accepted', $order->id) }}"
                                    class="d_accepted">Accept</a></button>
                    ...
                    @elseif($order->status == 5)
                        <div class="button ">
                            <div class="theme-bg btn nexBtn fs-25 mb-5 mt-5 btn-w"><a
                                    href="{{ route('mobile.delivery.complete_delivery', $order->id) }}">Pickup</div>
                        </div>
                    @endif
